...
xbmc_path = "plugin.video.youtube";
xbmc_autoplay = false;
...
function sendToXBMC(details) {
	if (typeof(safari) != "undefined") {
	    safari.self.tab.dispatchMessage("message",details);
	} else {
		self.postMessage("message",details); 
	}
}
...
function callJSONRpc(method, params, id) {
	...
	console.log("Calling XBMC with " + strData);
	sendToXBMC({
		method: 'POST',
...
function addButtonWithMenu() {
	...
	if ( menu ) {
		var xbmc_test = '<button type="button" id="xbmc-play-button" class="start yt-uix-tooltip-reverse addto-button-plus-hide-arrow yt-uix-button yt-uix-tooltip" onclick="" title="Play in XBMC" data-button-menu-id="" data-button-action="" role="button" aria-pressed="false"><img class="yt-uix-button-arrow" src="//s.ytimg.com/yt/img/pixel-vfl3z5WfW.gif" alt="">'
		//var xbmc_menu = '</button><button type="button" class="end yt-uix-tooltip-reverse yt-uix-button yt-uix-tooltip yt-uix-button-empty" onclick=";return false;" title="More XBMC Functions" data-button-menu-id="shared-xbmc-menu" data-button-action="" role="button" aria-pressed="false"><img class="yt-uix-button-arrow" src="//s.ytimg.com/yt/img/pixel-vfl3z5WfW.gif" alt=""></button><div id="shared-xbmc-menu" style="display: none;"><ul class="xbmc-menu"></ul>';
...
function messageHandler(response) {
	...
	xbmc_path = message.path || xbmc_path;
	xbmc_autoplay = message.autoplay || false;
...
function loadSettings(){
	if (safari) {
		...
		self.postMessage("getSettings",null); 
